...
#else
	#define ASAN_LOG(fmt, ...) do { } while (0)
#endif
...
#else
	#define ASAN_DEBUG_BREAK() do { } while (0)
#endif
...
#else
	#define DUMP_STACK_AT_LEVEL(p, x) do { } while (0)
#endif
...
static inline void NORETURN asan_internal_invalid_access(uptr p, size_t width, unsigned access_type) {
...
static inline asan_fakestack_config *asan_internal_fakestack_get_t_configs(asan_fakestack_config *config, uint32_t thread_id, size_t entryn) {
	return &config[thread_id * entryn];
}

static inline uint8_t *asan_internal_fakestack_get_t_entries(uint8_t *entries, uint32_t thread_id, size_t entryn, size_t entrysz) {
